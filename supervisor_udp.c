 ...
 * -- how to do a worst time analysis on a set of routines  
...
static char command[MAXLINE+1], *response;
...
static int max_priority; //, min_priority;
...
static void populate_routine_information(struct routine* myroutine, 
        char *routine_name, void *(*routine_pointer)(void*), double time,
        double period) 
{
    myroutine->routine_pointer = routine_pointer;
    myroutine->period = period;
    myroutine->deadline = period;
    myroutine->estimated_execution_time = time;
    myroutine->utilization = time / period;
...
static void populate_routine_list() 
{
    ...
    populate_routine_information(&routines_list[0], "fun1", &fun1,  2.0, 80.0); 
    populate_routine_information(&routines_list[1], "fun2", &fun2,  8.0, 40.0); 
    populate_routine_information(&routines_list[2], "fun3", &fun3,  8.0, 40.0);
    populate_routine_information(&routines_list[3], "fun3", &fun3,  9.0, 20.0);
    number_routines = 4;
    */

    /* Second set of routines:
    populate_routine_information(&routines_list[0], "fun1", &fun1,  2.0, 16.0); 
    populate_routine_information(&routines_list[1], "fun2", &fun2,  4.0, 40.0); 
    populate_routine_information(&routines_list[2], "fun3", &fun3, 20.0, 50.0);
    number_routines = 3;
    */

    /* Third set of routines:
    populate_routine_information(&routines_list[0], "fun1", &fun1, 10.0, 20.0); 
    populate_routine_information(&routines_list[1], "fun2", &fun2,  6.0, 30.0); 
    populate_routine_information(&routines_list[2], "fun3", &fun3, 10.0, 50.0);
    number_routines = 3;
    */

    /* Forth set of routines:
    populate_routine_information(&routines_list[0], "fun1", &fun1, 5.0,  20.0); 
    populate_routine_information(&routines_list[1], "fun2", &fun2, 10.0, 40.0); 
    populate_routine_information(&routines_list[2], "fun3", &fun3, 40.0, 80.0); 
    number_routines = 3;
    */
    
    /* Fifth set of routines:
    populate_routine_information(&routines_list[0], "fun1", &fun1, 3.0,  9.0); 
    populate_routine_information(&routines_list[1], "fun2", &fun2, 4.0, 12.0); 
    populate_routine_information(&routines_list[2], "fun3", &fun3, 2.0, 18.0); 
    number_routines = 3;
    */

    // Sixth set of routines:
    populate_routine_information(&routines_list[0], "fun1", &fun1, 3.0,  8.0); 
    populate_routine_information(&routines_list[1], "fun2", &fun2, 4.0, 14.0); 
    populate_routine_information(&routines_list[2], "fun3", &fun3, 5.0, 22.0); 
    populate_routine_information(&routines_list[3], "fun4", &fun4, 7.0, 30.0); 
    number_routines = 4;
}

// Returns 0 if ordered by period in ascending order. 
// Returns -1 otherwise.
...
// Returns 0 if the routine is schedulable (on that core).
// Returns -1 otherwise.
...
// Returns 0 if the routine is started correctly.
// Returns -1 otherwise.
static int start_routine()
{
    ...
    // the metadata.   
    // The priority is chosen so to make the scheduler be Rate Monotonic.
    // (smaller periods, higher priorities given how the array is formed) 
    pthread_attr_t routine_attr;
    set_realtime_attribute(
            &routine_attr, 
            SCHED_FIFO, 
            max_priority - 1 - parsed_command.routine_index, 
...
// Returns 0 if the routine is stopped correctly.
// Returns -1 otherwise.
...
// Returns 0 if the command has been parsed correctly.
// Returns -1 otherwise.
...
{
    int pcr;
    printf("INFO: Waiting to receive a command\n");

    if (-1 == (command_length = recvfrom(sd, (char *)&command[0], MAXLINE, 
            MSG_WAITALL, (struct sockaddr *) &accept_sin, 
            &accept_sin_length))) { }
    command[command_length] = '\0';
    printf("INFO: The command received is '%s'\n", command);

    if (0 == strcmp(command, "list")) {
        routines_listing = malloc(MAXLINE * number_routines);
        list_routines();
    ...
    if (-1 == sendto(sd, (const char *)&response[0], strlen(response), 
            MSG_CONFIRM, (const struct sockaddr *) &accept_sin, 
            accept_sin_length)) { }
...
static void connection_handler() 
{
    ...
    printf("INFO: Ready to accept commands\n");
...
static double routine_worst_time_analysis(struct routine *myroutine) 
{
...
static void routines_list_worst_time_analysis() 
{
    ...
    for (int i = 0; i < number_routines; i++) {
        routines_list[i].estimated_execution_time = 
                routine_worst_time_analysis(&routines_list[i]);    
    }

    for (int i = 0; i < number_routines; i++) {
        routines_list[i].estimated_execution_time *= 1.5;
        routines_list[i].utilization = 
                routines_list[i].estimated_execution_time 
                / routines_list[i].period;
    }
